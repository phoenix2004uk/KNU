...
IF SHIP:NAME="KSAT - LKO "+names[0] {
	...
	DoAccurateBurn(5,{
		PARAMETER deviation.
		RETURN IsWithin(SHIP:OBT:APOAPSIS, targetAp, deviation, 1).
	}).
	Notify("Cirularizing").
	...
	DoAccurateBurn(5,{
		PARAMETER deviation.
		RETURN IsWithin(SHIP:OBT:SEMIMAJORAXIS, targetSMA, deviation, 1).
	}).
	SET TARGET TO Vessel("KSAT - LKO "+names[names:LENGTH-1]).
}
ELSE {
	...
	DoAccurateBurn(5,{
		PARAMETER deviation.
		RETURN IsWithin(SHIP:OBT:APOAPSIS, TARGET:OBT:APOAPSIS, deviation, 1).
	}).
	...
	DoAccurateBurn(1,{
		PARAMETER deviation.
		RETURN IsWithin(SHIP:OBT:SEMIMAJORAXIS, TARGET:OBT:SEMIMAJORAXIS, deviation, 1).
	},6).
}
