...
function kerbin_return {PARAMETER M,P.
	LOCK THROTTLE TO 1.
	IF SHIP:OBT:HASNEXTPATCH AND SHIP:OBT:NEXTPATCH:BODY = Kerbin {
	...
	ELSE IF BODY = Kerbin {
		IF ALT:PERIAPSIS < 30000 {
			LOCK THROTTLE TO 0.
			M["next"]().
		}
	}
}
function kerbin_sci {PARAMETER M,P.
